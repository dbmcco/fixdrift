...
from fixdrift.contracts import format_default_contract_block
from fixdrift.drift import compute_fix_drift
from fixdrift.git_tools import get_git_root, get_working_changes
from fixdrift.specs import FixdriftSpec, extract_fixdrift_spec, parse_fixdrift_spec
from fixdrift.workgraph import Workgraph, find_workgraph_dir, load_tasks
...
def _maybe_write_log(wg: Workgraph, task_id: str, report: dict) -> None:
    ...
    if not findings:
        msg = "Fixdrift: OK (no findings)"
    else:
        kinds = ", ".join(sorted({str(f.get("kind")) for f in findings}))
        msg = f"Fixdrift: {score} ({kinds})"
        if recs:
...
def _maybe_create_followups(wg: Workgraph, report: dict) -> None:
    ...
    follow_id = f"drift-fix-{task_id}"
    title = f"fix-quality: {task_title}"
    recs = report.get("recommendations") or []
    action_lines = "\n".join([f"- {str(r.get('action') or '').strip()}" for r in recs if str(r.get("action") or "").strip()])
    if not action_lines:
        action_lines = "- Re-align fix work with repro evidence, root cause, and regression checks."

    desc = (
        "Run a bounded fix-quality pass to stop patch-on-patch drift.\n\n"
        "Context:\n"
        ...
        + "\n"
        + (report.get("_fixdrift_block") or "").strip()
        + "\n"
    )
...
def cmd_wg_check(args: argparse.Namespace) -> int:
    ...
    raw_block = extract_fixdrift_spec(description)
    if raw_block is None:
        report = {
            ...
            "score": "green",
            "spec": None,
            "telemetry": {"note": "no fixdrift block"},
            "findings": [],
            "recommendations": [],
        }
        if args.json:
    ...
    try:
        spec_raw = parse_fixdrift_spec(raw_block)
        spec = FixdriftSpec.from_raw(spec_raw)
        ...
        report = {
            ...
            "findings": [
                {
                    "kind": "invalid_fixdrift_spec",
                    "severity": "warn",
                    "summary": "fixdrift block present but could not be parsed",
                }
            ],
            "recommendations": [
                {
                    "priority": "high",
                    "action": "Fix the fixdrift TOML block so it parses",
                    "rationale": "Fixdrift can only evaluate fix-quality drift when it can read configuration.",
                }
            ],
        }
        report["_fixdrift_block"] = f"```fixdrift\n{raw_block}\n```"
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="fixdrift")
    ...
    check = wg_sub.add_parser("check", help="Check bug-fix quality drift (requires a fixdrift block in the task)")
